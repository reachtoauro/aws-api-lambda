...
var ep = process.env.DOMAIN_ENDPOINT;
//Strip off https:// from the endpoint (if exist)
...
const endpoint = ep;
const sessionId = '2347687368823-' + process.env.AWS_REGION;
const owner = 'Hub';
const solution = 'spl237v1';
const clusterSize = 'Small';
const uuid = '2e640c1c-c1f4-498f-a7df-9b67f71cbc03';
const anonymousData = 'Yes';
...
function handler(input, context, callback) {
  ...
  if (!endpoint || !sessionId || !owner) {
    LOGGER.log('ERROR', `check environment variables endpoint: ${endpoint}, sessionId: ${sessionId}, owner: ${owner}`);
    return callback('environment variables not defined');
  }

  LOGGER.log('INFO', `end point url: ${endpoint}`);
